...
zshTerminalCommand terminalName command =
    terminalName ++ " zsh -i -c " ++ command
...
makeNamedScratchPad ::
    String -> String -> String -> RationalRect -> NamedScratchpad
...
wideAccordionLayout =
    renamed [Replace "horizontalAccordion"] $ Mirror Accordion
...
myLayout = hiddenWindows $ avoidStruts $ noBorders Full ||| resizeableLayouts
  where
    resizeableLayouts =
        rawSpacing'
            10
        ...
        , ((modMask .|. shiftMask, xK_t), spawn "xfce4-taskmanager")
            ...
            ++ [
               ...
               ,
                   ...
                   , namedScratchpadAction myScratchPads "terminalfm"
                   )
               ,
                   ( (modMask .|. controlMask, xK_y)
                   , namedScratchpadAction myScratchPads "yt-music"
                   )
               ,
                   ( (modMask .|. controlMask, xK_m)
                   , namedScratchpadAction myScratchPads "emoji-picker"
                   )
               ,
            ...
            ++ [
               ...
               ,
                   ...
                   , spawn "~/.config/rofi/applets/android/powermenu.sh"
                   )
               ,
                   ( (modMask .|. shiftMask, xK_a)
                   , spawn "~/.config/rofi/launchers/ribbon/launcher.sh full_bottom"
                   )
               ,
                   ( (modMask .|. controlMask, xK_a)
                   , spawn "~/.config/rofi/launchers/ribbon/launcher.sh ribbon_bottom"
                   )
...
myConfig =
  ...
  where
    ...
    newMouseBindings xc =
        M.union (mouseBindings def xc) (M.fromList (myMouse xc))
